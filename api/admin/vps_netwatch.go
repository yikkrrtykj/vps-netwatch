...
type netwatchTargetRequest struct {
...
type mihomoDiscoverRequest struct {
	Controller string `json:"controller"`
	Secret     string `json:"secret"`
	Limit      int    `json:"limit"`
}

type mihomoTarget struct {
	Target  string   `json:"target"`
	Type    string   `json:"type"`
...
func AddNetwatchTarget(c *gin.Context) {
	var req netwatchTargetRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		api.RespondError(c, http.StatusBadRequest, err.Error())
		return
	}

	target, detectedType, err := normalizeNetwatchTarget(req.Target)
	if err != nil {
		api.RespondError(c, http.StatusBadRequest, err.Error())
		return
	}
	...
	clientIDs, err := normalizeNetwatchClients(req.Clients)
	if err != nil {
		api.RespondError(c, http.StatusBadRequest, err.Error())
		return
	}
...
func DiscoverMihomoTargets(c *gin.Context) {
	var req mihomoDiscoverRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		api.RespondError(c, http.StatusBadRequest, err.Error())
		return
	}
	...
	if err != nil || baseURL.Host == "" {
		api.RespondError(c, http.StatusBadRequest, "invalid mihomo controller address")
		return
	}
	...
	if err != nil {
		api.RespondError(c, http.StatusBadGateway, "failed to query mihomo: "+err.Error())
		return
	}
	...
	if err != nil {
		api.RespondError(c, http.StatusBadGateway, "failed to read mihomo response: "+err.Error())
		return
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		api.RespondError(c, http.StatusBadGateway, fmt.Sprintf("mihomo returned %d: %s", resp.StatusCode, strings.TrimSpace(string(body))))
		return
	}
	...
	if err := json.Unmarshal(body, &payload); err != nil {
		api.RespondError(c, http.StatusBadGateway, "invalid mihomo response: "+err.Error())
		return
	}
	...
	discovered := make(map[string]*mihomoTarget)
	...
	targets := make([]mihomoTarget, 0, len(discovered))
...
func normalizeNetwatchTarget(raw string) (string, string, error) {
	...
	if host, port, ok, err := splitNetwatchHostPort(input); err != nil {
		return "", "", err
	} else if ok {
...
func splitNetwatchHostPort(input string) (string, string, bool, error) {
...
func normalizeNetwatchClients(raw []string) ([]string, error) {
	if len(raw) > 0 {
